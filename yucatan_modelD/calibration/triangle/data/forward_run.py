...
def plot_results(gwf, headFile_path: Path, title, layer, out_png, cbar_label):
    ...
    contour = pmv.contour_array(h[layer][0], ax=ax, levels=10, colors='black')
...
def main():
    ...
    for layer in (0,1):
        plot_results(gwf, headfile, f"Caso D - Calibrado\nMalla triangular - Capa {int(layer)+1}", layer, root / f"tri_casoD_calibrado_layer{int(layer)+1}", "Cargas hidráulicas [m.s.n.m.]")
