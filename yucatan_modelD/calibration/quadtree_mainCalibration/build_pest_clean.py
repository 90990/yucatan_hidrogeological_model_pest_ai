...
def main():
    ...
    if sgd_obs_path.exists():
        sgd = pd.read_csv(sgd_obs_path, dtype={"seg_id": str})
        # make a stable obs name list
        sgd_obs_names = [f"sgd_{sid}" for sid in sgd["seg_id"].astype(str).tolist()]

        # write instruction file: one float per line
        with open(pest_dir / "sim_sgd.ins", "w", encoding="utf-8", newline="\n") as f:
            f.write("pif ~\n")
            for on in sgd_obs_names:
                f.write(f"l1 !{on}!\n")

    # seed output file if missing: one value per line
    if not (pest_dir / "sim_sgd.dat").exists():
        with open(pest_dir / "sim_sgd.dat", "w", encoding="utf-8") as f:
            for _ in sgd_obs_names:
                f.write("0.0\n")
