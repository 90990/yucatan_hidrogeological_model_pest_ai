\
# forward_run.py
# Run the MF6 forward model for PEST++ and write model outputs required by instruction files.
#
# Outputs expected (depending on your PEST setup):
#   - sim_heads.dat : simulated heads at observation wells (obs_id, sim_head)
#   - sim_sgd.dat   : simulated SGD per coastline segment (sgd_###, m/d) [optional]
...
# def write_sim_heads_dat(root: Path) -> None:
#     """Write sim_heads.dat with lines: '<obs_id> <sim_head>' (no header)."""
#     obs_def = pd.read_csv(root / "obs_heads.csv")
#     if "obs_id" not in obs_def.columns or "cellid" not in obs_def.columns:
#         raise ValueError("obs_heads.csv must have columns: obs_id, cellid (and optionally others).")

#     # load heads from last time
#     hds_path = root / "gwf.hds"
#     if not hds_path.exists():
#         # fallback: any *.hds
#         hds_hits = list(root.glob("*.hds"))
#         if not hds_hits:
#             raise FileNotFoundError("No head file (*.hds) found.")
#         hds_path = hds_hits[0]

#     hf = flopy.utils.HeadFile(hds_path)
#     times = hf.get_times()
#     totim = times[-1]
#     h = np.array(hf.get_data(totim=totim))
#     h_flat = h.ravel()

#     # write file
#     out = root / "sim_heads.dat"
#     with open(out, "w", encoding="utf-8") as f:
#         for _, r in obs_def.iterrows():
#             oid = str(r["obs_id"])
#             cid = int(r["cellid"])
#             if cid < 0 or cid >= h_flat.size:
#                 raise IndexError(f"cellid {cid} out of range for head array length {h_flat.size}.")
#             f.write(f"{oid} {float(h_flat[cid]):.6f}\n")
