...
def apply_parameters(gwf, pars: dict[str, float], root: Path) -> None:
    ...
    if not (K_base.size == zone_upland.size):
        raise ValueError(
            f"Size mismatch: K_base({K_base.size}), upland({zone_upland.size}), rest({zone_rest.size}). "
    ...
    K_new[sl1] = mK_rest_l1
    K_new[sl2] = mK_rest_l2
...
def plot_results(gwf, headFile_path: Path, title, layer, out_png, cbar_label):
    ...
    contour = pmv.contour_array(h_layer, ax=ax, levels=3, colors='black')
...
def main() -> None:
    ...
    pars = read_params(root / "absolute_params_caseA.csv")
    ...
    for layer in (0,1):
        plot_results(gwf, headfile, f"Caso A - Calibrado\nMalla estructurada - Capa {int(layer)+1}", layer, root / f"estruc_casoA_calibrado_layer{int(layer)+1}", "Cargas hidráulicas [m.s.n.m.]")
