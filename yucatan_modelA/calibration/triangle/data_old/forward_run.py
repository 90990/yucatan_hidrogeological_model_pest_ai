...
def apply_parameters(gwf, pars: dict[str, float], root: Path) -> None:
    ...
    karr = npf.k.array
    if karr.ndim != 2:
        raise ValueError(f"Expected NPF.k.array to have 2 dimensions (nlay, ncpl). Got {karr.shape}")

    nlay,ncpl = karr.shape
    if nlay < 2:
        raise ValueError(f"Expected at least 2 layers. Got {nlay}")
    if zone_upland.size != ncpl:
        raise ValueError(f"zone_upland size {zone_upland.size} != ncpl {ncpl}")
    
    K2D = np.empty((nlay, ncpl), dtype=float)
    ...
    if rch is not None and spd_path.exists():
        ...
        if rch_arr.size != ncpl:
            raise ValueError(f"RCH array size {rch_arr.size} != ncpl {ncpl}")
...
def extract_sim_heads(headfile_path: Path, obs_cellids_path: Path, gwf) -> pd.DataFrame:
    ...
    h = np.squeeze(hf.get_data(totim=totim))
    #h = hf.get_data()[0][0]

    if h.ndim != 2:
        raise ValueError(f"Expected heads 2D (nlay, ncpl) for DISV, Got {h.shape}")

    #layer = obs["layer"].astype(int).to_numpy - 1 # Layer column is 1-based in file
    cellid = obs["cellid"].astype(int).to_numpy()  # Must be 0-based
...
def plot_results(gwf, headFile_path: Path, title, layer, out_png, cbar_label):
    ...
    hd = pmv.plot_array(h[layer][0], ax=ax, alpha=1.0, cmap='viridis')
    contour = pmv.contour_array(h[layer][0], ax=ax, levels=5, colors='black')
...
def main():
    ...
    pars = read_params(root / "absolute_params1_caseA.csv")
    ...
    write_sim_sgd_segments_dat(root)
    for layer in (0,1):
        plot_results(gwf, headfile, f"Caso A - Calibrado\nMalla triangular - Capa {int(layer)+1}", layer, root / f"tri_casoA_calibrado_layer{int(layer)+1}", "Cargas hidráulicas [m.s.n.m.]")
