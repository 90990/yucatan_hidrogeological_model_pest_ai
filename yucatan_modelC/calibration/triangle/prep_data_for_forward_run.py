...
Run:
  python prep_template_clean.py --base_model_dir "C:/path/to/your/mf6_run" --obs_heads_csv "C:/path/to/obs_heads.csv" 
           --upland_cellids_npy "C:/path/to/upland_cellids.npy"
"""
...
def _build_zone_masks_disu_disv(gwf, upland_cellids_0based: np.ndarray):
    mg = gwf.modelgrid
    if isinstance(mg, StructuredGrid):
        raise ValueError("This model is Structured Grid (DIS)")

    if isinstance(mg, UnstructuredGrid):
        raise ValueError("This model is a Unstructured Grid (DISU/DISV)")
...
def export_base_arrays_and_masks(gwf, template_dir: Path, zone_upland: np.ndarray):
...
def main():
    ...
    shutil.copy2(root / "absolute_params1_caseC.csv", data_dir / "absolute_params1_caseC.csv")
    ...
    zone_upland = _build_zone_masks_disu_disv(gwf,upland_cellids)

    export_base_arrays_and_masks(gwf, data_dir, zone_upland)
